...
public class GameEngine {
	...
	private void initializeTimer() {
		// Schedule the timer task to update the timer label every second
		sessionTimer.scheduleAtFixedRate(new TimerTask() {
			...
			public void run() {
				if (secondsRemaining > 0) {
					...
					// End the game or move to the next level when time runs out
					if (currentLevel < 3) {
						nextLevel();
					} else {
						...
						// Close the GameGUI or perform other necessary actions
					}
				}
			}
		}, 0, 1000);
